...
/**
 * Verifica se o usuário tem perfil de Residente
 * @returns {Promise<boolean>} Promise com resultado da verificação
 */
export const checkResidentProfile = async () => {
    try {
        const response = await apiClient.get(`${process.env.NEXT_PUBLIC_DIRECTUS_URL}/users/me?fields=*.*`);
        ...
        if (response.data && response.data.data && response.data.data.role && response.data.data.role.name) {
            ...
            return response.data.data.role.name === "Residente";
        }

        return false;
    } catch (error) {
        console.error("Erro ao verificar perfil de residente:", error);
        throw error;
    }
};
